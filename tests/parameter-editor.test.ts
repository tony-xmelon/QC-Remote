...
test("parameter scene navigation uses the QC reference double chevrons", () => {
  ...
  const rasterSource = readFileSync(new URL("../packages/typescript/qc-theme/src/reference-icon-rasters.ts", import.meta.url), "utf8");
  ...
  assert.match(rasterSource, /"editor\.scene-previous"/);
  assert.match(rasterSource, /"editor\.scene-next"/);
