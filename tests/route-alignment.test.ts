...
const SIDES = [
  { side: "input" as const, message: "GainCalInputPortParameter", enumName: "InputPortId", routes: QC_INPUT_ROUTES, contract: domain.inputRoutes, cells: "ROUTE_INPUT_CELLS", captions: ["MONO", "STEREO", "OTHER"] },
  { side: "output" as const, message: "GainCalOutputPortParameter", enumName: "OutputPortId", routes: QC_OUTPUT_ROUTES, contract: domain.outputRoutes, cells: "ROUTE_OUTPUT_CELLS", captions: ["STEREO", "MONO", "OTHER"] }
];

for (const { side, message, enumName, routes, contract, cells, captions } of SIDES) {
  ...
  test(`the ${side} route selector draws every selectable device route once`, () => {
    const fixtureRows = fixtureCells(cells);
    assert.deepEqual(fixtureRows.filter((row) => captions.includes(row)), captions,
      `the ${side} route popup captions changed order`);
    const labels = new Set(Object.values(DEVICE_LABELS[side]).filter((label): label is string => label !== null));
    const selectableRows = fixtureRows.filter((row) => !captions.includes(row));
    assert.equal(new Set(selectableRows).size, selectableRows.length, `${side} route rows must not be duplicated`);
    assert.deepEqual(new Set(selectableRows), labels,
      `the ${side} route popup must cover the protocol-backed selectable route labels`);
    for (const row of selectableRows) {
      assert.ok(labels.has(row), `the ${side} popup shows a route the protocol contract cannot name: ${row}`);
    }
  });
}
