...
const pythonType = (schema) => {
  ...
  const base = type === "boolean" ? "bool"
    : type === "integer" ? "int"
      : type === "number" ? "float"
        : type === "string" ? "str"
          : type === "array" && schema.items?.type === "object" ? "list[dict[str, int]]"
            : type === "array" ? `list[${pythonType(schema.items)}]`
              : undefined;
  if (!base) throw new Error(`No Python MCP annotation mapping for ${JSON.stringify(schema)}`);
...
${actions.map((action) => `    def ${action.name}(
        self,
${Object.keys(action.properties).map((name) => `        ${name}: ${pythonType(action.inputSchema.properties[name])},`).join("\n")}
    ) -> Any:
...
`;
const rustInlineValues = (values) => values.map((value) => JSON.stringify(value)).join(", ");
const rustNumber = (value) => Number.isInteger(value) ? `${value}.0` : String(value);
const rustMaximum = (schema, number = false) => schema.maximum === undefined
  ? (number ? "Some(f64::MAX)" : "None") : `Some(${number ? rustNumber(schema.maximum) : schema.maximum})`;
const rustKind = (schema) => {
  const types = Array.isArray(schema.type) ? schema.type : [schema.type];
  const nullable = types.includes("null");
  const type = types.find((candidate) => candidate !== "null");
  if (type === "string") {
    const values = schema.enum?.filter((value) => value !== null);
    if (values) return `Kind::${nullable ? "NullableStringEnum" : "StringEnum"}(&[${rustInlineValues(values)}])`;
    if (schema.pattern) return `Kind::${nullable ? "NullableVisibleString" : "VisibleString"} { max_chars: ${schema.maxLength} }`;
    return nullable ? "Kind::NullableString" : "Kind::String";
  }
  if (type === "boolean") return nullable ? "Kind::NullableBoolean" : "Kind::Boolean";
  if (type === "integer") {
    if (schema.enum) return `Kind::IntegerEnum(&[${rustInlineValues(schema.enum)}])`;
    const range = `min: ${schema.minimum ?? 0}, max: ${rustMaximum(schema)}`;
    return `Kind::${nullable ? "NullableInteger" : "Integer"} { ${range} }`;
  }
  if (type === "number") {
    const range = `min: ${schema.minimum === undefined ? "-f64::MAX" : rustNumber(schema.minimum)}, max: ${rustMaximum(schema, true)}`;
    return `Kind::${nullable ? "NullableNumber" : "Number"} { ${range} }`;
  }
  if (type === "array" && nullable) {
    return `Kind::NullableStringArray { max_items: ${schema.maxItems}, values: &[${rustInlineValues(schema.items.enum)}] }`;
  }
  if (type === "array" && schema.items.type === "object") return "Kind::MidiMessages";
  if (type === "array" && schema.items.type === "boolean") return "Kind::BooleanRows";
  if (type === "array" && schema.items.type === "integer") {
    return `Kind::IntegerArray { min: ${schema.items.minimum}, max: ${schema.items.maximum}, min_items: ${schema.minItems}, max_items: ${schema.maxItems}, unique: ${schema.uniqueItems} }`;
  }
  throw new Error(`No Rust MCP kind mapping for ${JSON.stringify(schema)}`);
};
...
pub static ACTIONS: &[ActionSpec] = &[\n${actions.map((action) => `    ActionSpec {\n\
        ...
        properties: &[${Object.keys(action.properties).map((name) => `${action.required.includes(name) ? "p!" : "p!"}(${action.required.includes(name) ? "" : "? "}${JSON.stringify(name)}, ${rustKind(action.inputSchema.properties[name])})`).join(", ")}],\n\
