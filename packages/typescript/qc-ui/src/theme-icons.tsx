...
function QcReferenceRasterLayers({ icon, className }: { icon: ReferenceRasterName; className?: string }) {
  const raster = QC_REFERENCE_ICON_RASTERS[icon];
  const shapeRendering = icon === "editor.scene-previous" || icon === "editor.scene-next" || icon === "editor.copy" || icon === "editor.paste" || icon === "editor.reset" ? "auto" : "crispEdges";
...
export function QcModeGlyph({ mode }: { mode: PresetSnapshot["mode"] }) {
  ...
  const raster = QC_REFERENCE_ICON_RASTERS[icon];
  return <svg width={mode === "HYBRID" ? raster.width : 24} height={mode === "HYBRID" ? raster.height : 24} viewBox={`0 0 ${raster.width} ${raster.height}`} className={`qc-mode-glyph qc-mode-glyph-${mode.toLowerCase()}`} shapeRendering="crispEdges" aria-hidden="true">
...
export function QcLibraryIcon({ kind, className }: { kind: QcLibraryIconName; className?: string }) {
  ...
  if (kind === "capture-library") return <QcReferenceRaster icon="library.capture-library" color={QC_COLORS.captured.iconPrimary} className={classes} />;
  ...
  if (kind === "impulse-response") return <QcReferenceRaster icon="library.impulse-response" color={QC_COLORS.captured.iconPrimary} className={classes} />;
...
export function QcEditorIcon({ kind }: { kind: QcEditorIconName }) {
  ...
  if (referenceIcon) return <QcReferenceRasterLayers icon={referenceIcon} className={`qc-editor-icon qc-editor-icon-${kind}`} />;
  if (kind === "looper") {
