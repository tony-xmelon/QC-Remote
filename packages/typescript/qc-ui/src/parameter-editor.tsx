  ...
  return <section className={`ir-loader-channel${muteValue >= .5 ? " is-bypassed" : ""}`} style={{ "--ir-accent": accent } as CSSProperties}>
    <div className="ir-loader-selector">
      <div className="ir-loader-state">
        <button className={muteValue < .5 ? "is-active" : ""} disabled={disabled || !mute?.writable} aria-label={`${muteValue >= .5 ? "Enable" : "Disable"} impulse`} aria-pressed={muteValue < .5} onClick={() => commitBinary(mute, muteValue >= .5 ? 0 : 1)}><QcUiIcon kind="power" /></button>
        <button className={phaseValue >= .5 ? "is-active" : ""} disabled={disabled || !phase?.writable} aria-label="Invert impulse phase" aria-pressed={phaseValue >= .5} onClick={() => commitBinary(phase, phaseValue >= .5 ? 0 : 1)}><QcUiIcon kind="phase-invert" /></button>
      </div>
...
function CabChannel({ side, allParameters, drafts, accent, disabled, onDraftChange, onCommit, onCommitBatch }: {
  ...
  return <section className={`cab-channel${bypassValue >= .5 ? " is-bypassed" : ""}`} style={{ "--cab-accent": side ? QC_COLORS.category.pitch : accent, "--mic-x": `${10 + positionValue * 80}%`, "--mic-y": `${24 + distanceValue * 66}%` } as CSSProperties}>
    ...
    <div className="cab-channel-footer">
      <button className={`cab-phase${phaseValue >= .5 ? " is-active" : ""}`} disabled={disabled || !phase?.writable} aria-label={`Invert microphone ${side + 1} phase`} aria-pressed={phaseValue >= .5} onClick={() => setBinary(phase, phaseValue >= .5 ? 0 : 1)}><QcUiIcon kind="phase-invert" /></button>
      ...
      <button className={`cab-channel-power${bypassValue < .5 ? " is-active" : ""}`} disabled={disabled || !bypass?.writable} aria-label={`${bypassValue >= .5 ? "Enable" : "Disable"} cab microphone ${side + 1}`} aria-pressed={bypassValue < .5} onClick={() => setBinary(bypass, bypassValue >= .5 ? 0 : 1)}><QcUiIcon kind="power" /></button>
    </div>
  </section>;
}
