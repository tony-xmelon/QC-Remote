...
// The plugin directory's logo and the device browser's empty pane draw the
// same zigzag - 105x124 at (527, 194) in `device-favorites.png`.
function DeviceWaveformGlyph({ className }: { className: string }) {
  return <svg className={className} viewBox="0 0 120 140" aria-hidden="true"><path d="M0 70H18L42 2 75 138 94 70H120" /></svg>;
}
...
function CorOsRemainingFixture({ view }: { view: RemainingFixtureView }) {
  ...
  if (view === "fixture-editor-capture") return <section className="qc-screen capture-editor-physical" aria-label="Neural Capture editor">
    ...
    <div className="capture-editor-grid"><span>In<br />1</span><i className="capture-cable" /><i className="capture-block is-gate">⌁</i><i className="capture-block is-capture">▤</i><i className="capture-block">▤</i><span>Row<br />3</span></div>
    <section className="capture-editor-panel"><header><button><QcUiIcon kind="more" /></button><span><small>NEURAL CAPTURE</small><strong>Keeley Mod BD-2 3</strong></span><nav><i>◀◀</i><b>A</b><i>▶▶</i></nav><button className="capture-editor-power"><ExpressionPowerIcon /></button><button className="capture-editor-confirm"><QcUiIcon kind="check" /></button></header><main>{[["GAIN","-3.8 dB",-28],["BASS","1.0 dB",18],["MID","0.0 dB",0],["TREBLE","1.6 dB",28],["VOLUME","-3.8 dB",-28]].map(([label,value,angle])=><section key={label as string}><span>{label}</span><i><b style={{transform:`rotate(${angle}deg)`}} /></i><strong>{value}</strong></section>)}</main></section>
  </section>;
...
function CorOsOfficialSettings({ view }: { view: "settings-account" | "settings-system" | "settings-device" | "settings-midi" }) {
  const data = view === "settings-midi"
    ...
    ? { title: "Account", icon: "♧", active: 0, rows: [["♙", "My Account"], ["♻", "Backups"]] }
  ...
  return <section className={`qc-screen coros-settings-official ${view}`} aria-label={`${data.title} Settings`}>
    ...
    <main><nav>{data.rows.map(([icon, label], index) => <button key={label} className={index === data.active ? "is-active" : ""}><b>{view === "settings-account" ? <SettingsAccountGlyph kind={index === 0 ? "user" : "backup"} /> : view === "settings-device" ? <SettingsDeviceIcon label={label} /> : icon}</b>{label}</button>)}</nav>
      {view === "settings-account" ? <section className="settings-account-detail is-my-account"><h1>Device linked to [redacted]</h1><p>{"You are ready to send & receive Presets & Neural Captures and use Cloud backups."}</p><button>UNLINK DEVICE</button></section>
...
function CorOsIoSettings({ initialView, onClose }: { initialView: IoView; onClose: () => void }) {
  ...
  return <section className={`coros-io-settings is-${view}`} aria-label={`I/O Settings ${title}`}>
    ...
    <div className="io-ports">{IO_PORTS.map((port, index) => <button key={`${port.label}-${index}`} className={`${port.id === view && (view !== "input" || index === IO_PORTS.length - 1) ? "is-active" : ""} is-${port.kind ?? "jack"}${port.paired ? " is-paired" : ""}`} onClick={() => setView(port.id)}><span className={port.id === "headphones" ? "io-headphone-label" : undefined}>{port.id === "headphones" ? <IoHeadphonesGlyph /> : port.label}</span><i><IoPortGlyph kind={port.kind ?? "jack"} primary={index === IO_PORTS.length - 1 && view === "input"} /></i>{port.paired && <i><IoPortGlyph /></i>}<small>{port.sub}</small></button>)}{view === "usb" && <div className="io-input-selectors"><button>1</button><button>2</button></div>}</div>
...
function CorOsGlobalEq({ onClose }: { onClose: () => void }) {
  ...
  return <section className="coros-global-eq" aria-label="Global EQ">
    <header><button className="global-eq-more">⋮</button><span><small>GLOBAL EQ</small><strong>Parametric-5</strong></span><button className="global-eq-power is-off"><i /> OFF</button><button aria-label="Close Global EQ" onClick={onClose}>✓</button></header>
    <div className="global-eq-graph"><div>{verticals.map((left) => <i key={left} style={{ left: `${left}%` }} />)}</div><svg viewBox="0 0 800 255" preserveAspectRatio="none"><g className="eq-axis-labels"><text x="208" y="13">100</text><text x="471" y="13">1k</text><text x="736" y="13">10k</text></g><path d="M0 125H800" /><g>{[105, 210, 447, 606, 671].map((x, index) => <g key={x}><circle cx={x} cy={125} r={index === 0 ? 27 : 21} className={index === 0 ? "is-active" : ""} /><text x={x} y={130}>{index + 1}</text></g>)}</g></svg></div>
    <div className="global-eq-tabs">{[1,2,3,4,5].map((tab) => <button key={tab} className={tab === 1 ? "is-active" : ""}>{tab}</button>)}<button>OUT</button></div>
    <div className="global-eq-controls"><section><span>TYPE</span><button>⌁　LO SHELF　⌄</button></section>{[["GAIN","0.0 dB"],["FREQ","50 Hz"],["Q","0.71"]].map(([label,value]) => <section key={label}><span>{label}</span><IoDial value={value} /></section>)}<section className="global-eq-bypass"><span>BYPASS 1</span><button><ExpressionPowerIcon /></button></section></div>
  </section>;
}
...
function CorOsRoutingScreen({ view, snapshot }: { view: "splitter-placement" | "splitter-editor" | "mixer-editor" | "empty-slot"; snapshot: PresetSnapshot }) {
  if (view === "empty-slot") return <section className="qc-screen empty-slot-official" aria-label="Empty-slot device browser"><nav className="empty-slot-categories">{COROS_DEVICE_CATEGORIES.slice(0, 6).map(([label, glyph, color], index) => <button key={label} style={{ "--device-color": color } as CSSProperties}><i><DeviceCategoryGlyph label={label} fallback={glyph} /></i><span>{label}</span>{index === 0 && <b>New</b>}</button>)}</nav><section className="empty-slot-grid"><header><strong><span>4</span>E</strong><h1>QC MCP TEST_2</h1><span className="empty-undo"><GridToolbarIcon kind="undo" /></span><b>A</b><span className="empty-save"><GridToolbarIcon kind="save" /></span><span className="empty-more"><GridToolbarIcon kind="more" /></span></header><div className="empty-mode"><GridToolbarIcon kind="mode" /><strong>PRESET</strong></div><main><span className="empty-route">In<br />1</span><span className="empty-route is-row-3">In<br />1</span><i className="empty-cable" /><i className="empty-cable is-row-3" /><i className="empty-add">＋</i><i className="empty-row is-row-2">＋</i><i className="empty-row is-row-4">＋</i><i className="empty-out">Multi<br />Out</i></main></section></section>;
