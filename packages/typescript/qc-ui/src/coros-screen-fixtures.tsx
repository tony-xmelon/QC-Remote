...
function CorOsOfficialDirectory({ view, filter = false }: { view: OfficialDirectoryView; filter?: boolean }) {
  ...
  if (view === "directory-search-results") {
    ...
    return <section className="qc-screen directory-official directory-search-official"><header><button className="directory-search-field"><DirectoryIcon kind="search" /><span>Brit</span></button><button className="directory-search-tab is-active"><QcModeGlyph mode="PRESET" /><span>(15)</span></button><button className="directory-search-tab"><CaptureHeaderIcon /><span>(50)</span></button><button className="directory-search-tab"><QcLibraryIcon kind="impulse-response" /><span>(0)</span></button><span /><button><DirectoryIcon kind="arrange" /></button><button><DirectoryIcon kind="done" /></button></header><main><section className="directory-official-list"><header>DEVICE DIRECTORIES <b><QcUiIcon kind="collapse" /></b></header>{rows.map(name => <button key={name}><span>{name}<small>Neural DSP</small></span><b>B　<QcUiIcon kind="more" /></b></button>)}</section></main></section>;
  }
