...
function CorOsRemainingFixture({ view }: { view: RemainingFixtureView }) {
  ...
  if (view === "fixture-editor-capture") return <section className="qc-screen capture-editor-physical" aria-label="Neural Capture editor">
    ...
    <section className="capture-editor-panel"><header><button><QcUiIcon kind="more" /></button><span><small>NEURAL CAPTURE</small><strong>Keeley Mod BD-2 3</strong></span><nav><i>◀</i><b>A</b><i>▶</i></nav><button aria-label="Bypass"><QcEditorIcon kind="bypass" /></button><button aria-label="Confirm"><QcEditorIcon kind="confirm" /></button></header><main>{[["GAIN","-3.8 dB",-28],["BASS","1.0 dB",18],["MID","0.0 dB",0],["TREBLE","1.6 dB",28],["VOLUME","-3.8 dB",-28]].map(([label,value,angle])=><section key={label as string}><span>{label}</span><i><b style={{transform:`rotate(${angle}deg)`}} /></i><strong>{value}</strong></section>)}</main></section>
  </section>;
